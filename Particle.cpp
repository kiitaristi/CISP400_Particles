	...
	: m_A(2, numPoints)
{
	...
	m_vx = (float)((rand() % 300 - 100 + 1) + 100);
	m_vy = (float)((rand() % 500 - 100 + 1) + 100);

	// Randomize particles' horizontal movement
	m_vx *= (rand() % 2 == 0) ? 1 : -1;
...
void Particle::draw(RenderTarget& target, RenderStates states) const
{
	...
	{
		Vector2f m_matrixSet(m_A(0, j - 1), m_A(1, j - 1));
		Vector2f m_pixelSet(target.mapCoordsToPixel(m_matrixSet, m_cartesianPlane));

		lines[j].position = m_pixelSet;
